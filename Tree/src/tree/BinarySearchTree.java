...
{  if (!withinView(o))
   ...
   {  // find where to add newNode
      ...
      {  int comparison = compare(o, currentNode.element);
      	nodeVisited(currentNode); //node is visited
...
{  boolean removed = false;
   ...
   {  // check if root to be removed
	   //if rootNode, no need to clone 
      if (compare(element, rootNode.element)==0)
      {  rootNode = makeReplacement(rootNode);
      	nodeVisited(rootNode); //node is visited
         removed = true;
      }
      else if(contains(o)) //check if this element is actually in the tree
      {  
         ...
         {  // determine whether the removalNode has been found
            ...
            nodeVisited(parentNode); //node is visited
            if (comparison==0)
...
{  BinaryTreeNode replacementNode = null;
   ...
   {  // find the inorder successor and use it as replacementNode
      ...
      replacementNode = removalNode.rightChild;
      nodeVisited(replacementNode); //node is visited
...
//this method have same structure of contains method 
//but returns BinaryTreeNode object
public BinaryTreeNode getNode(Object o) {
	boolean found = false;
	   E element = (E)o; // unchecked, could throw exception
	   if (!withinView(element))
		   return null;
...
//this hook method create a list that needs to be copied
//for Persistent Dynamic Set
public void nodeVisited(BinaryTreeNode node) {
	visitedList.add(node);
}

//this method creates new BinaryTreeNode with parameter input
public BinaryTreeNode createNode(E element) {
...
public class BinaryTreeNode 
{
   ...
   {  this.element= element;
      leftChild = null;
      rightChild = null;
      color = null; //color is only used for Red Black Tree(Balanced Tree)
   }
