...
public class AllPairsFloydWarshall
{
   ...
   public double[][][] d; //d[k][i][i] is weight of path from v_i to v_j
   public int[][][] p; //p[k][i][i] is penultimate vertex in path
         ...
         {  for (int j=0; j<n; j++)
            {  double s;
               if (d[k-1][i][k-1]!=INFINITY&&d[k-1][k-1][j]!=INFINITY) {
            	    s = d[k-1][i][k-1] + d[k-1][k-1][j]; //association is multiplying
