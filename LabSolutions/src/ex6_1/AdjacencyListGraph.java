...
{  // first add the end vertices if not already in graph
   ...
   //save hashcode to find duplicated vertex
   hashCode[count] = vertex0.hashCode();
   hashCode[count+1] = vertex1.hashCode();

   //find duplicate hashcode
   //if found throw exception
   for(int i=0; i<=(count+1); ++i) {
	   for(int j=i+1; j<=(count+1); ++j) {
		   if(hashCode[i] == hashCode[j]) {
			   System.out.println("More than one edge cannot be exist between two vertexes!\n");
			   throw new IllegalArgumentException();
		   }
	   }
   }
   //if duplicate not found create the new edge
