...
public class BinarySearchTree<E> extends AbstractSet<E> implements SortedSet<E>{
	...
	//the set, and returns true if sorted set did not already
		...
		if(rootNode == null) {
			...
			while(!done) {
				...
				{	if (currentNode.rightChild==null)
					...
					done = true; //no duplicates in this binary tree
			}
		}
	...
	private int compare(E element1, E element2) {
		if(comparator != null)
			...
			return ((Comparable)element1).compareTo(element2); //unchcked
		else if(element2!=null && element2 instanceof Comparable)
			return -((Comparable)element2).compareTo(element1); //unchcked
		else return 0;
		}
	...
	public boolean remove (Object o) {
		...
		E element = (E)o; //unchcked, could throw exception
	...
	//helper method which removes removalNode (presumed not null)
	...
	//overriden method with an efficient O(log n) search algorithm
	//rather than the superclasses O(n)linear search using iteator
	...
	//ist at least as big as fromElement, returns null if none found
	@Override
	public E first() {
		if(rootNode==null)
	...
	private class BinaryTreeIterator implements Iterator<E>{
		...
		public BinaryTreeIterator(BinaryTreeNode rootNode) {
			//puts the elements in a linked lst using inorder traversal
			list = new LinkedList<E>();
			//change this to other method to change the orders
